...
`formatter` first-hit prefers `.formatter.exs` over `prettier.config.*` / `.prettierrc*` / `biome.json` when `mix.exs` and `.formatter.exs` exist. When `Gemfile` and `.rubocop.yml` / `.rubocop.yaml` / `.standard.yml` exist, first-hit prefers those over prettier/biome. A Mix tree with only prettier still passes. A Rails tree with only prettier still passes. A JS-only prettier tree still passes.
